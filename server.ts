...
async function startServer() {
  ...
  // Setup Gemini SDK dynamically per generate request to guarantee latest API Key is resolved
  console.log("Back-end initialized. API Key will be resolved dynamically during generate requests.");

  // API Endpoint: Query Knowledge Base manually
    ...
    try {
      ...
      if (!apiKey) {
        return res.status(500).json({
          error: "GEMINI_API_KEY is not configured in the Secrets panel of Google AI Studio (Settings > Secrets). Please verify your API Key setting and restart.",
        });
      }
